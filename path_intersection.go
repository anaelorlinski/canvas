	...
	for _, square := range squares {
		...
		BuildPath:
			...
			for {
				...
				if next == nil {
					// Upstream PR #382: close partial contours to the
					// other endpoint instead of panicking. The previous
					// behaviour was to panic on dense shared-edge
					// geometry (e.g. stroke outlines clipped against
					// rects with coincident edges). Visually this can
					// leave a slightly-incorrect contour, but it's
					// vastly preferable to crashing the whole render.
